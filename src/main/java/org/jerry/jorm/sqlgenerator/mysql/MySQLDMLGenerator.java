 ...
 */
public class MySQLDMLGenerator implements SQLGenerator {
    ...
    public static void r(Filterable filterable, EntityDescriptor entityDescriptor, StringBuffer filterSql, List<Filter> filters) {
        if (filterable == null) {
            return;
        }
        if (filterable instanceof Filter) {
            ...
            filters.add(filter);
            filterSql.append(filter2Sql(entityDescriptor, filter, filters.size() - 1));
        } else {
            ...
            boolean parentheses = expression.isParentheses();
            if (parentheses) {
                filterSql.append(" (");
            }
            List<Expression.LogicOperator> logicOperatorList = expression.getLogicOperators();
    ...
    public static String filter2Sql(EntityDescriptor entityDescriptor, Filter filter, int i) {
